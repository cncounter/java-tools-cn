...
的`jstatd`命令是一个监视器的RMI服务器应用程序的创建和终止插装Java HotSpot vm和提供了一个接口,使远程监控工具来连接到jvm 在本地主机上运行。
...
的`jstatd`在本地主机服务器需要一个RMI注册。的`jstatd`服务器尝试连接到RMI注册在默认端口,或在您所指定的端口`-p` `port`选择。如果没有找到RMI注册,然后创建一个内`jstatd`应用程序绑定到端口所表示的`-p` `port`选择或默认RMI注册中心港口时`-p` `port`选择是省略。你可以停止创建一个内部RMI注册通过指定`-nr`选择。
