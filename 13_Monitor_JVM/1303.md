...
如果没有安装其他的安全管理器, 则`jstatd`服务器会安装一个 `RMISecurityPolicy` 实例, 因此,需要指定一个安全策略文件(security policy file)。策略文件必须符合默认的 Policy 实现规范, 以及 Policy 文件语法格式。
...
界面导出`jstatd`过程是专有的和保证改变。用户和开发人员劝阻写入这个接口。
...
`jstatd`命令。`jstatd`在后台自动启动服务器的脚本
